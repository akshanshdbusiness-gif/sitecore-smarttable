...
import type {
  ...
  SmartTableProps,
  SmartTableRowItem,
} from './smart-table.props';

/**
 * SmartTable — renders a table stored as Row/Cell child items of the datasource.
 *
...
const cellText = (cell: SmartTableCellItem): string => cell.cellContent?.jsonValue?.value ?? '';
...
export const Default: React.FC<SmartTableProps> = (props) => {
  ...
  if (!datasource) {
    return isPageEditing ? (
      <div className="rounded border border-dashed border-gray-300 p-6 text-center text-sm text-gray-500">
        SmartTable: no datasource selected. Choose or create one in the component
        properties.
      </div>
    ) : null;
  }
  ...
  if (allRows.length === 0) {
    return isPageEditing ? (
      <div className="rounded border border-dashed border-gray-300 p-6 text-center text-sm text-gray-500">
        SmartTable is empty. In Content Editor, insert <strong>SmartTableRow</strong>{' '}
        items under this datasource, then <strong>SmartTableCell</strong> items
        inside each row.
      </div>
    ) : null;
  }
  ...
  const renderRow = (row: SmartTableRowItem, index: number, header: boolean) => {
  ...
  return (
    <div className={['smart-table', params?.GridParameters, params?.Styles].filter(Boolean).join(' ')}>
